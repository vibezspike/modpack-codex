(function(){
  ...
  function renderGrid(recipe){
    ...
    cells.forEach(ch => {
      if (ch === ' ' || !recipe.key[ch]) {
        ...
        html += `<div class="cslot filled">${escapeHtml(recipe.key[ch])}</div>`;
      }
    });
    html += `</div>`;
    return html;
  ...
  function renderFlow(recipe){
    ...
    html += `<div class="io-pill-group"><div class="io-pill guaranteed">${escapeHtml(recipe.flowInput)}</div></div>`;
    html += `<div class="flow-arrow">→</div>`;
    html += `<div class="io-pill-group">`;
    recipe.flowOutputs.forEach(o => {
      const cls = o.guaranteed ? 'io-pill guaranteed' : 'io-pill';
      let label = escapeHtml(o.text);
  ...
  function renderRecipeCard(recipe){
    ...
    return `
      <div class="card" data-search="${escapeHtml((recipe.output + ' ' + recipe.id + ' ' + (recipe.note||'')).toLowerCase())}">
        <div class="card-head">
